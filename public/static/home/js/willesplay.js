$(function() {
	...
	playVideo.on('ended', function() {
		$('.playTip').removeClass('am-icon-pause').addClass('am-icon-play').fadeIn();
	...
	function volumeControl(e) {
		...
		if (percentage < 0) {
			percentage = 0;
			$('.otherControl .volume').attr('class', 'volume glyphicon am-icon-volume-off');
		}
		if (percentage > 50) {
			$('.otherControl .volume').attr('class', 'volume glyphicon am-icon-volume-up');
		}
		if (percentage > 0 && percentage <= 50) {
			$('.otherControl .volume').attr('class', 'volume glyphicon am-icon-volume-down');
		}
		if (percentage >= 100) {
			percentage = 100;
		}
	...
	function playControl() {
			...
			if (playVideo[0].paused) {
				playVideo[0].play();
				$('.playTip').removeClass('am-icon-play').addClass('am-icon-pause').fadeOut();
			} else {
				playVideo[0].pause();
				$('.playTip').removeClass('am-icon-pause').addClass('am-icon-play').fadeIn();
			}
