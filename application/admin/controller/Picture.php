...
class Picture  extends AdminBase
{
    ...
    {
    	$category_level_list = Db::name('Picture')->order('id desc')->paginate('15');
    ...
    public function cate_edit()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $shu['title'] = $data['name'];
            $shu['img'] = $data['thumb'];
            $list = Db::name('picture')->insert($shu);
            if ($list) {
     ...
     public function cate_edits($id){
        if ($this->request->isPost()) {
            ...
            $shu['title'] = $data['name'];
            $shu['img'] = $data['thumb'];
            $list = Db::name('picture')->where('id',$data['id'])->update($shu);
            if ($list) {
                $this->success('保存成功',url('index'));
            }
            $this->error('保存失败');
